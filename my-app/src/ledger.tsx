...
function CustomerLedger() {
  ...
  const fetchCustomerSales = (customerId: any) => {
    axios
      .get(`${API_URL}/api/customer-sales/${customerId}/`)
      .then((res) => {
        setSales(res.data);
      })
      .catch(() => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        if (id) {
          const selected = res.data.find((c: any) => c.id == id);
  ...
  const totalDebt = Array.isArray(sales)
  ...
  const payDebt = async () => {
    ...
    try {
      await axios.post(`${API_URL}/api/pay-debt/${selectedSale.id}/`, {
        amount,
      ...
      const updatedSales = sales.map((sale: any) => {
        if (sale.id === selectedSale.id) {
          const updatedPaid = Number(sale.paid) + amount;

          const updatedBalance = Number(sale.balance) - amount;
      ...
      const updatedSelected = updatedSales.find(
        (s: any) => s.id === selectedSale.id,
      );
                    ...
                    <td>
                      ...
                      {sale.payments?.map((payment: any, i: number) => (
                        <div key={i} className="payment-history">
                          Paid ₦{formatMoney(payment.amount)} via{" "}
                          <strong>{payment.payment_method}</strong> on{" "}
